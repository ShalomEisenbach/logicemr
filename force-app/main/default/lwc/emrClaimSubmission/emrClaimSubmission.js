...
export default class EmrClaimSubmission extends LightningElement {
    ...
    get canSubmit() {
        return this.claimStatus === 'Ready' && !this.hasPending && !this.hasSubmitted;
    }
    ...
    async handleSubmit() {
        ...
        if (!confirmed) return;
        await this.run(() => queueSubmission({ superbillId: this.superbillId }), 'Claim submission queued.');
    }
