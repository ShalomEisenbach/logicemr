...
const EDIT_DEMOGRAPHICS = "edit_demographics";
...
export default class EmrPatientSearch extends NavigationMixin(LightningElement) {
	...
	get columns() {
		return [
			...
			{label: "Status", fieldName: STATUS_FIELD.fieldApiName},
			{
				...
				typeAttributes: {
					rowActions: [
						{label: "Open chart", name: OPEN_CHART},
						{label: "Edit demographics", name: EDIT_DEMOGRAPHICS}
					]
				}
			}
		];
	...
	get patientCards() {
		return (this.patients || []).map((row) => {
			const name = [row[LAST_NAME_FIELD.fieldApiName], row[FIRST_NAME_FIELD.fieldApiName]].filter((part) => part).join(", ");
			const parts = [row[MRN_FIELD.fieldApiName], row[PHONE_FIELD.fieldApiName], row[EMAIL_FIELD.fieldApiName], formatDob(row[DOB_FIELD.fieldApiName]), row[STATUS_FIELD.fieldApiName]].filter((part) => part);
			return {
				id: row.Id,
				title: name || "Patient",
				meta: parts.join(" · "),
	...
	handleEditCard(event) {
		this.openEditDemographics(event.currentTarget.dataset.id);
	}

	openEditDemographics(patientId) {
		const registration = this.template.querySelector("c-emr-patient-registration");
		if (registration && typeof registration.openForEdit === "function") {
			registration.openForEdit(patientId);
		}
	}

	handleRowAction(event) {
		const actionName = event.detail.action.name;
		if (actionName === EDIT_DEMOGRAPHICS) {
			this.openEditDemographics(event.detail.row.Id);
			return;
		}
		if (actionName !== OPEN_CHART) {
			return;
		}
		this[NavigationMixin.Navigate]({
			type: "standard__recordPage",
